#!/bin/bash
# ──────────────────────────────────────────────────────────────
# run_v1p2_experiment.sh – LoRA v1.2 GOOGL Bias Correction
# LoRA v1.2 GOOGL 偏置修正实验流水线
#
# Usage:
...
CONFIG="configs/lora_v1p2.yaml"
PROMPT="prompts/zero_shot_plain.txt"
ADAPTER="checkpoints/lora_v1p2_seed${SEED}"
TAG="lora_v1p2_seed${SEED}"

echo "═══════════════════════════════════════════════════"
echo "  LoRA v1.2 Experiment Pipeline (GOOGL Bias Fix)"
echo "  Mode: ${MODE} | Seed: ${SEED}"
echo "  Train: train_v2p2.jsonl (488 + 32 GOOGL patch)"
echo "═══════════════════════════════════════════════════"
echo ""

# ── Step 1: Training ──────────────────────────────────
if [[ "$MODE" == "full" ]]; then
    echo "▶ Step 1: Full LoRA v1.2 training (seed=$SEED)..."
    ...
    echo "Usage: bash scripts/run_v1p2_experiment.sh {full|eval} [seed]"
    exit 1
fi
...
python scripts/evaluate.py \
    ...
    --json-output "results/v1p2_test_seed${SEED}.json"
...
python scripts/evaluate.py \
    ...
    --json-output "results/v1p2_ambiguous_seed${SEED}.json"
...
python scripts/evaluate.py \
    ...
    --json-output "results/v1p2_hard_seed${SEED}.json"
...
python scripts/evaluate.py \
    ...
    --json-output "results/v1p2_google_shareclass_seed${SEED}.json"
...
python scripts/evaluate.py \
    ...
    --json-output "results/v1p2_meta_alias_seed${SEED}.json"

# ── Done ──────────────────────────────────────────────
echo ""
echo "═══════════════════════════════════════════════════"
echo "  ✓ LoRA v1.2 Experiment Complete (seed=$SEED)"
echo "═══════════════════════════════════════════════════"
echo ""
echo "  Outputs:"
echo "    Adapter:       $ADAPTER"
echo "    Predictions:   data/*_${TAG}_preds.jsonl"
echo "    Main eval:     results/v1p2_{test,ambiguous,hard}_seed${SEED}.json"
echo "    Diagnostics:   results/v1p2_{google_shareclass,meta_alias}_seed${SEED}.json"
